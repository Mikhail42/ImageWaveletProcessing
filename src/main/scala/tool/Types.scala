...
object Types {
  ...
  type P = (T, T)
  def pairToP: Pair => (T, T) = (p: Pair) => (p.e1, p.e2)
  def pToPair: ((T, T)) => Pair = (p: P) => new Pair(p._1, p._2)
  case class Pair(e1: T, e2: T){
    ...
    def / (z: T): Pair = this*(1/z)
    def sqrNorm: T  = this*this
    def - (p: Pair): Pair = Pair(e1-p.e1, e2-p.e2)
    def toP: (T, T) = (e1, e2)
  }
  
  case class ArPair(p1: Pair, p2: Pair){
    def * (p: Pair): Pair = Pair(p1*p, p2*p)
    def / (e: T): ArPair = ArPair(p1/e, p2/e)
  }
}